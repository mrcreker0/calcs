...
Обозначим
\[
\Delta x_i=x_{i+1}-x_i,\qquad \lambda(T)=\max_{0\leqslant i\leqslant n-1}\Delta x_i.
\]
Для набора точек $\xi_i\in[x_i,x_{i+1}]$ сумма
\[
\sigma(T,\xi)=\sum\limits_{i=0}^{n-1} f(\xi_i)\,\Delta x_i
\]
...
\begin{definition}
Пусть $f$ ограничена на $[a,b]$ и $ f \in R[a,b]$. Тогда для любого разбиения
\[
T:\quad a=x_0<x_1<\dots<x_n=b.
\]
Обозначим
...
\[
 s(T)=\sum\limits_{i=0}^{n-1} m_i\,\Delta x_i,\qquad
 S(T)=\sum\limits_{i=0}^{n-1} M_i\,\Delta x_i.
\]
\end{definition}
...
\[
 s(T)\leqslant \sigma(T,\xi)\leqslant S(T).
\]
\end{prop}
...
\[
S(T')\leqslant S(T),\qquad s(T')\geqslant s(T).
\]
\end{prop}
...
\[
\boxed{s \leqslant \underline I \leqslant \overline I \leqslant S} (*)
\]
\[
...
Тогда
\[
S(T)-s(T)=\sum\limits_{i=0}^{n-1}\omega_i\,\Delta x_i.
\]
\end{remark}
...
\]
Так как $s(T)\leqslant \sigma(T,\xi)\leqslant S(T)$, то
\[
I-\frac{\varepsilon}{2}\leqslant s(T)\leqslant S(T)\leqslant I+\frac{\varepsilon}{2}.
\]
Следовательно,
\[
0\leqslant S(T)-s(T)\leqslant \varepsilon,
\]
...
\[
0\leqslant \overline I-\underline I\leqslant S(T)-s(T)\to 0,
\]
следовательно, $\underline I=\overline I=:I$.
Из свойства Дарбу получаем
\[
s(T)\leqslant I\leqslant S(T),
\]
а значит для любой интегральной суммы
\[
0\leqslant |\sigma(T,\xi)-I|\leqslant S(T)-s(T)\to 0.
\]
Следовательно, $\lim\limits_{\lambda(T)\to 0}\sigma(T,\xi)=I$, то есть $f\in R[a,b]$.
\end{proof}
...
Тогда
\[
 s(T)=0,\qquad S(T)=\sum\limits_{i=0}^{n-1}\Delta x_i=b-a,
\]
то есть $S(T)-s(T)=b-a\neq 0$.
...
Тогда
\[
0\leqslant S(T)-s(T)=\sum\limits_{i=0}^{n-1}\omega_i\,\Delta x_i
<\sum\limits_{i=0}^{n-1}\frac{\varepsilon}{b-a}\,\Delta x_i
...
поэтому
\[
\omega_i=M_i-m_i\leqslant f(b)-f(a).
\]
Следовательно,
\[
S(T)-s(T)=\sum\limits_{i=0}^{n-1}\omega_i\,\Delta x_i
\leqslant \bigl(f(b)-f(a)\bigr)\sum\limits_{i=0}^{n-1}\Delta x_i
